...
class WC_Appointment_Form {
	...
	private function staff_field() {
		// Staff field
		if ( ! $this->product->has_staff() || 'customer' != $this->product->get_staff_assignment_type() ) {
			return;
		}
		...
		foreach ( $staff as $staff_member ) {
			$additional_cost = '';
			$cost_plus_base  = ( $staff_member->get_base_cost() + $this->product->price );
			$additional_cost = array();

			if ( $staff_member->get_base_cost() && $this->product->get_base_cost() < $cost_plus_base ) {
			...
			$staff_options[ $staff_member->ID ] = $staff_member->display_name . apply_filters( 'woocommerce_appointments_staff_additional_cost_string', $additional_cost_string, $staff_member );
		}

		$this->add_field( array(
			'type'    => 'select-staff',
			'name'    => 'staff',
			'label'   => $this->product->wc_appointment_staff_label ? $this->product->wc_appointment_staff_label : __( 'Providers', 'woocommerce-appointments' ),
	...
	public function output() {
		...
		foreach ( $this->fields as $key => $field ) {
			wc_get_template(
				'appointment-form/' . $field['type'] . '.php', array( 'field' => $field ), '', WC_APPOINTMENTS_TEMPLATE_PATH );
		}
	}
	...
	public function get_posted_data( $posted = array() ) {
		...
		if ( $this->product->has_staff() ) {
			if ( $this->product->is_staff_assignment_type( 'customer' ) && ! empty( $posted['wc_appointments_field_staff'] ) ) {
				...
				if ( $staff = $this->product->get_staff() ) {
					...
					$data['staff']     = '';
					foreach ( $staff as $staff_member ) {
						$data['_staff_ids'][] = $staff_member->ID;
						$data['_staff_names'][] = $staff_member->display_name;
					}
		...
		return apply_filters( 'woocommerce_appointments_get_posted_data', $data, $this->product );
	}
	...
	public function is_appointable( $data ) {
		...
		if ( $this->product->has_staff() && $this->product->is_staff_assignment_type( 'customer' ) ) {
			if ( empty( $data['_staff_id'] ) ) {
				// return new WP_Error( 'Error', sprintf( __( 'Please choose the %s.', 'woocommerce-appointments' ), $this->product->wc_appointment_staff_label ? $this->product->wc_appointment_staff_label : __( 'Providers', 'woocommerce-appointments' ) ) );
				$data['_staff_id'] = 0;
			}
		...
		if ( $min = $this->product->get_min_date() ) {
			$min_date = $this->product->get_min_timestamp_for_date( strtotime( $data['_date'] ) );
