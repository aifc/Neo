...
class WC_Appointments_Integration_Addons {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_product_addons_panel_head_option', array( $this, 'addon_option_head' ), 10, 2 );
		add_action( 'woocommerce_product_addons_panel_body_option', array( $this, 'addon_option_body' ), 10, 3 );
	 ...
	 * Show option head
	 */
	public function addon_option_head( $addon, $loop ) {
		$product = wc_get_product( get_the_ID() );
		$css_classes = 'duration_column show_if_appointment';
		if ( $product && 'appointment' !== $product->product_type ) {
			$css_classes .= ' hide_initial_appointment_addon_options';
		}
	 ...
	 * Show option body
	 */
	public function addon_option_body( $addon, $loop = 0, $option = array() ) {
		$product = wc_get_product( get_the_ID() );
		$css_classes = 'duration_column show_if_appointment';
		if ( $product && 'appointment' !== $product->product_type ) {
			$css_classes .= ' hide_initial_appointment_addon_options';
