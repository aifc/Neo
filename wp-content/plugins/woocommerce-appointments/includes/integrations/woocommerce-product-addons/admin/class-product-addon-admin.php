...
class Product_Addon_Admin {
	...
	public function styles() {
		wp_enqueue_style( 'woocommerce_product_addons_css', WC_APPOINTMENTS_PLUGIN_URL . '/includes/integrations/woocommerce-product-addons/assets/css/admin.css' );
	}
	...
	public function global_addons_admin() {
		if ( ! empty( $_GET['add'] ) || ! empty( $_GET['edit'] ) ) {
			...
			if ( ! empty( $_GET['edit'] ) ) {
				...
				$product_terms_objects = get_the_terms( $global_addon->ID, apply_filters( 'woocommerce_product_addons_global_post_terms', 'product_cat' ) );
				$objects 		= wp_list_pluck( (array) $product_terms_objects, 'term_id' );
	 ...
	 */
	public function tab() {
		?><li class="addons_tab product_addons"><a href="#product_addons_data"><?php _e( 'Add-ons', 'woocommerce-appointments' ); ?></a></li><?php
	}
	...
	private function get_posted_product_addons() {
		...
		if ( isset( $_POST['product_addon_name'] ) ) {
			...
			$addon_size = count( $addon_name );

			 for ( $i = 0; $i < $addon_size; $i++ ) {
				...
				$option_size = count( $option_label );

				for ( $ii = 0; $ii < $option_size; $ii++ ) {
