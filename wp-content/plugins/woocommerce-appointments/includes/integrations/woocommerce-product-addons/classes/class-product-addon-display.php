...
class Product_Addon_Display {
	...
	public function addon_scripts() {
		...
		wp_register_script( 'accounting', WC()->plugin_url() . '/assets/js/accounting/accounting' . $suffix . '.js', array( 'jquery' ), WC_APPOINTMENTS_VERSION );

		wp_enqueue_script( 'woocommerce-addons', WC_APPOINTMENTS_PLUGIN_URL . '/includes/integrations/woocommerce-product-addons/assets/js/addons.js', array( 'jquery', 'accounting' ), WC_APPOINTMENTS_VERSION, true );
	...
	public function quick_view_single_compat() {
		if ( is_singular( 'product' ) && class_exists( 'WC_Quick_View' ) ) {
			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
			wp_enqueue_script( 'woocommerce-addons-quickview-compat', WC_APPOINTMENTS_PLUGIN_URL . '/includes/integrations/woocommerce-product-addons/assets/js/quickview' . $suffix . '.js', array( 'jquery' ), WC_APPOINTMENTS_VERSION, true );
		}
	}
	...
	public function totals( $post_id ) {
		...
		if ( is_object( $the_product ) ) {
			...
			$display_price    = 'incl' == $tax_display_mode ? $the_product->get_price_including_tax() : $the_product->get_price_excluding_tax();
		} else {
			$display_price    = '';
			$raw_price        = 0;
		}
	...
	public function add_to_cart_url( $url ) {
		...
		if ( ! is_single( $product->id ) && in_array( $product->product_type, apply_filters( 'woocommerce_product_addons_add_to_cart_product_types', array( 'subscription', 'simple' ) ) ) && ( ! isset( $_GET['wc-api'] ) || 'WC_Quick_View' !== $_GET['wc-api'] ) ) {
			if ( $this->check_required_addons( $product->id ) ) {
				if ( version_compare( WOOCOMMERCE_VERSION, '2.5.0', '<' ) ) {
					$product->product_type = 'addons';
				}
