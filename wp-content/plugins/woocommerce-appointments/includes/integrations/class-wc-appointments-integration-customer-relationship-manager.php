...
class WC_Appointments_Integration_CRM {
	...
	public function wca_crm_use_crm_customer_edit_url( $appointment_id ) {
	    ?>
			...
			jQuery('.wca-customer-url, .wca-edit-customer' ).attr('href','<?php echo admin_url(); ?>admin.php?page=wc_crm&c_id=' + customer_crm_id);
			...
			if(customer_notes !== undefined && customer_notes !== ''){
				...
				jQuery('#wca-detail-customer dd .dashicons-testimonial').tipTip({content: '<u><?php echo __( 'Customer Notes', 'woocommerce-appointments' ); ?></u><ol>' + customer_notes + '</ol>'});
			}
			...
			if(order_notes !== undefined && order_notes !== ''){
				...
				jQuery('#wca-detail-staff dd .dashicons-testimonial').tipTip({content: '<u><?php echo __( 'Order Notes', 'woocommerce-appointments' ); ?></u><ol>' + order_notes + '</ol>'});
			}
