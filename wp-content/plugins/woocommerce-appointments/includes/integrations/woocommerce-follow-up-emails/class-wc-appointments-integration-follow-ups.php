...
class WC_Appointments_Integration_WCFUE {
    ...
    function __construct() {
        ...
        // trigger fields
        add_filter( 'fue_actions_table_rows', array( $this, 'add_product_selector' ) );
        ...
        add_action( 'fue_followup_form_after_trigger_row', array( $this, 'followup_form_triggers' ), 9, 3 );
    ...
    public function register_email_type( $types ) {
        ...
        $duration_units = method_exists( 'Follow_Up_Emails', 'get_schedule_units' ) ? Follow_Up_Emails::get_schedule_units() : $durations;
        ...
        $props = array(
            ...
            'triggers'              => $triggers,
            'durations'             => Follow_Up_Emails::get_schedule_units(),
        ...
        $types[] = new FUE_Type( 'wc_appointments', $props );
    ...
    public function manual_email_recipients( $recipients, $post ) {
        ...
        if ( 'scheduled_event' == $post['send_type'] ) {
            $search_args = array(
    ...
    public function add_product_selector( $email ) {
        if ( in_array( $email->type, apply_filters( 'fue_appointment_form_products_selector_email_types', array( 'wc_appointments' ) ) ) ) {
            ...
            include( 'templates/email-form/appointments/followup-form.php' );
        }
    }
    ...
    public function email_form_script() {
        wp_enqueue_script( 'fue-form-appointments', WC_APPOINTMENTS_PLUGIN_URL . '/assets/js/followup-form-appointments.js', array( 'jquery' ), WC_APPOINTMENTS_VERSION );
    }
    ...
    public function email_variables_list( $email ) {
        ...
        <li class="var hideable var_wc_appointments"><strong>{appointment_id}</strong> <img class="help_tip" title="<?php _e( 'The ID of scheduled product or service', 'woocommerce-appointments' ); ?>" src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" width="16" height="16" /></li>
        <li class="var hideable var_wc_appointments"><strong>{appointment_cancellation_link}</strong> <img class="help_tip" title="<?php _e( 'The cancellation link for appointment', 'woocommerce-appointments' ); ?>" src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" width="16" height="16" /></li>
    <?php
    }
    ...
    protected function add_variable_replacements( $variables, $email_data, $queue_item, $email ) {
        if ( $queue_item->order_id && $queue_item->product_id ) {
            ...
            $appointment_staff              = ( $staff = $appointment->get_staff_members( $names = true ) ) ? $staff : '';
            $appointment_id                 = $appointment->id;
            $appointment_cancellation_link  = $appointment->get_cancel_url();
    ...
    public function appointment_created( $appointment_id ) {
        ...
        // stop FUE from scheduling blank emails after adding an appointment product to the cart
    ...
    public function appointment_status_updated( $appointment_id ) {
        ...
        if ( 'paid' == $status || 'confirmed' == $status ) {
        ...
        // see if appointment date has changed and update the scheduled emails if any
        if ( 'paid' == $status && ! empty( $_POST['appointment_start_date'] ) ) {
            $start_date = get_post_meta( $appointment_id, '_appointment_start', true );
            $posted_start_date = str_replace( '-', '', $_POST['appointment_start_date'] );

            if ( $start_date != $posted_start_date ) {
                $this->update_scheduled_emails( $appointment_id, $posted_start_date );
            }
        }
    ...
    public function maybe_trigger_status_update( $post_id ) {
        if ( ! empty( $_POST['post_type'] ) && 'wc_appointment' != $_POST['post_type'] ) {
            return $post_id;
        }
     ...
     *
     * @param FUE_Followup $followup
     */
    public function followup_form_triggers( FUE_Followup $followup ) {
        if ( 'wc_appointments' == $followup->type ) {
    ...
    public function get_orders_for_email( $orders, $email ) {
        ...
        if ( ( $status = array_search( $email->trigger, $status_triggers ) ) ) {
            ...
            $posts_query = new WP_Query();
            $appointment_posts = $posts_query->query( $posts_args );
        ...
        if ( 'wc_appointments' == $email->type ) {
            foreach ( $data as $email_id => $orders ) {
                foreach ( $orders as $idx => $appointment_id ) {
                    ...
                    // An appointment can have no order linked to it
                    if ( $order ) {
                        if ( Follow_Up_Emails::instance()->fue_wc->wc_scheduler->exclude_based_on_categories( $order, $email ) ) {
                            unset( $data[ $email_id ][ $idx ] );
                            continue;
                        }
    ...
    private function create_email_order( $appointment_id, $triggers = array() ) {
        ...
        foreach ( $emails as $email ) {
            ...
            if ( 'date' == $email->duration ) {
                ...
                if ( 'before_appointment_event' == $email->interval_type ) {
                    $start  = strtotime( get_post_meta( $appointment_id, '_appointment_start', true ) );
                    $time   = FUE_Sending_Scheduler::get_time_to_add( $email->interval_num, $email->interval_duration );

                    $send_on = $start - $time;
                } elseif ( 'after_appointment_event' == $email->interval_type ) {
                    $start  = strtotime( get_post_meta( $appointment_id, '_appointment_end', true ) );
                    $time   = FUE_Sending_Scheduler::get_time_to_add( $email->interval_num, $email->interval_duration );
            ...
            if ( ! is_wp_error( FUE_Sending_Scheduler::queue_email( $insert, $email ) ) ) {
                ...
                if ( $order ) {
                    ...
                    $note = sprintf(
                        __( 'Email queued: %1$s scheduled on %2$s<br/>Trigger: %3$s', 'woocommerce-appointments' ),
                        $email->name,
                        $send_date,
                        $email_trigger
                    );
    ...
    /**
     * Update appointment reminders using the new appointment date
     *
     * @param int       $appointment_id
     * @param string    $new_appointment_date
     */
    private function update_scheduled_emails( $appointment_id, $new_appointment_date ) {
        ...
        $followups = fue_get_followups( 'any', 'any', '', array(
            'meta_query'    => array(
                array(
                    'key'       => '_interval_type',
                    'value'     => array( 'before_appointment_event' ),
                    'compare'   => 'IN',
                ),
            ),
        ) );
        ...
        foreach ( $followups as $followup ) {
            // look for unsent items in the queue whose schedule need updating
            $queue = $scheduler->get_items(array(
                'email_id'      => $followup->id,
                'is_sent'       => 0,
                'order_id'      => $appointment->order_id,
                'product_id'    => $appointment->product_id,
            ));
            ...
            foreach ( $queue as $item ) {
                if ( 'before_appointment_event' == $followup->trigger  ) {
                    $start  = strtotime( $new_appointment_date . '000000' );
                    $time   = FUE_Sending_Scheduler::get_time_to_add( $followup->schedule_value, $followup->schedule_unit );
                    ...
                    if ( $appointment->order_id ) {
                        $order = $appointment->get_order();
                        $order->add_order_note( sprintf( __( 'Email %1$s rescheduled to %2$s', 'follow_up_emails' ), $followup->name, fue_format_date( $item->send_on, true ) ) );
                    }
                }
            }
        }
