...
class WC_Appointment_Cart_Manager {
	...
	public function add_to_cart() {
		...
		// Get template
		wc_get_template( 'single-product/add-to-cart/appointment.php', array( 'appointment_form' => $appointment_form ), '', WC_APPOINTMENTS_TEMPLATE_PATH );
	}
	...
	public function schedule_cart_removal( $appointment_id ) {
		$minutes = apply_filters( 'woocommerce_appointments_remove_inactive_cart_time', 60 );
		...
		wp_schedule_single_event( $timestamp, 'wc-appointment-remove-inactive-cart', array( $appointment_id ) );
	}
	...
	public function validate_appointment_requires_confirmation( $passed, $product_id ) {
		if ( wc_appointment_requires_confirmation( $product_id ) ) {
			...
			wc_add_notice( __( 'An appointment that requires confirmation has been removed from your cart. It is not possible to complete the purchased along with an appointment that doesn\'t require confirmation.', 'woocommerce-appointments' ), 'notice' );
		}
