...
class WC_Appointments_Admin_Settings extends WC_Settings_Page {
	...
	public function output() {
		...
		} elseif ( 'notify' == $current_section ) {
			include( 'views/html-settings-send-notifications.php' );
		} elseif ( 'gcal' == $current_section ) {
	...
	public function save_global_availability() {
		...
		if ( ! empty( $_POST['appointments_availability_submitted'] ) ) {
			...
			// Validate appointments if there is any rule mismatch include all statuses but complete.
			$current_date = date( 'YmdHis' );
			$latest_appointments_args = array(
			...
			$latest_appointments = WC_Appointments_Controller::get_appointments( $latest_appointments_args );
			foreach ( $latest_appointments as $latest_appointment ) {
				WC_Appointments_Controller::validate_appointment( $latest_appointment );
			}
		}
	}
				...
				$appointments_args = array(
					'meta_query' => array(
						array(
							...
							'value' => absint( $notification_product_id ),
						),
					),
					'post_status' => array( 'confirmed', 'paid' ),
				);
				$appointments        = WC_Appointments_Controller::get_appointments( $appointments_args );
				$mailer              = WC()->mailer();
