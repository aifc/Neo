		...
		try {
			...
			if ( ! empty( $_POST['add_appointment'] ) ) {
				...
				if ( $order_id ) {
					...
					// Add custom billing and shipping data
					if ( $customer_id ) {
						$get_addresses = array(
							'billing_first_name',
							'billing_last_name',
						...
						foreach ( $get_addresses as $name ) {
							update_post_meta( $order_id, '_' . $name, get_user_meta( $customer_id, $name, true ) );
						}
					}
	...
	public function create_order( $total, $customer_id ) {
		if ( function_exists( 'wc_create_order' ) ) {
			...
			$order_data = apply_filters( 'woocommerce_new_order_data', array(
				...
				'post_title' 	=> sprintf( __( 'Order &ndash; %s', 'woocommerce-appointments' ), strftime( _x( '%1$b %2$d, %Y @ %I:%M %p', 'Order date parsed by strftime', 'woocommerce-appointments' ) ) ),
				'post_status' 	=> 'publish',
				'ping_status'	=> 'closed',
				'post_excerpt' 	=> '',
				'post_author' 	=> 1,
