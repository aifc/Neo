...
class WC_Appointments_Admin_Ajax {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_wc-appointment-cancel', array( $this, 'mark_appointment_cancelled' ) );
	 ...
	 */
	public function mark_appointment_cancelled() {
		if ( ! current_user_can( 'manage_appointments' ) ) {
			wp_die( __( 'You do not have sufficient permissions to access this page.', 'woocommerce-appointments' ) );
		}
		if ( ! check_admin_referer( 'wc-appointment-cancel' ) ) {
			wp_die( __( 'You have taken too long. Please go back and retry.', 'woocommerce-appointments' ) );
		}
		$appointment_id = isset( $_GET['appointment_id'] ) && (int) $_GET['appointment_id'] ? (int) $_GET['appointment_id'] : '';
		if ( ! $appointment_id ) {
	...
	public function calculate_costs() {
		...
		$product    = wc_get_product( $appointment_id );
	...
	public function get_time_slots_for_date() {
		...
		if ( $product->get_interval_unit() && $product->get_interval() ) {
			$base_interval = 'hour' === $product->get_interval_unit() ? $product->get_interval() * 60 : $product->get_interval();
		}

		// Filters for the intervals.
		$interval = apply_filters( 'woocommerce_appointments_interval', $interval, $product, $timestamp );
		$base_interval = apply_filters( 'woocommerce_appointments_base_interval', $base_interval, $product, $timestamp );

		$from				= $time_from = strtotime( 'midnight', $timestamp );
		$to					= strtotime( 'midnight +1 day', $timestamp ) + $interval;
		...
		} elseif ( ( $staff = $product->get_staff() ) && count( $staff ) === 1 ) {
		...
		$slot_html = $product->get_available_slots_html( $slots, array( $interval, $base_interval ), $time_to_check, $staff_id_to_check, $from, $timezone );
	...
	public function json_search_order() {
		...
		echo wp_json_encode( $found_orders );
		die();
	}
