	...
	<div id="wca-dialog-wrap" class="wp-core-ui" role="dialog" aria-labelledby="wca-dialog-title">
		<form id="wca-dialog">
			<div id="wca-dialog-header">
				<h1><?php _e( 'Add Appointment', 'woocommerce-appointments' ); ?></h1>
					...
					<dl id="wca-new-appointment-product">
						...
						<dd>
							<select id="appointable_product_id" name="appointable_product_id" class="chosen_select" style="width:300px">
								<option value=""><?php _e( 'Select a product &#8230;', 'woocommerce-appointments' ); ?></option>
								<?php foreach ( get_wc_appointment_products() as $product ) : ?>
									<option value="<?php echo $product->ID; ?>" data-duration="<?php echo get_post_meta( $product->ID, '_wc_appointment_duration', true ) ?>" data-duration-unit="<?php echo get_post_meta( $product->ID, '_wc_appointment_duration_unit', true ); ?>" ><?php echo $product->post_title; ?></option>
								<?php endforeach; ?>
							</select>
						</dd>
					</dl>
					<dl id="wca-new-appointment-staff">
						<dt><?php _e( 'Staff:', 'woocommerce-appointments' ); ?></dt>
						<dd>
							<select id="select_staff" name="select_staff" class="chosen_select" style="width:300px">
								<option value=""><?php _e( 'Select staff &#8230;', 'woocommerce-appointments' ); ?></option>
			...
			<div id="wca-dialog-footer">
				...
				<div id="wca-dialog-update">
					<button type="submit" id="wca-dialog-new-appointment-submit" name="wca-dialog-new-appointment-submit" class="button button-primary">
						<?php _e( 'Add Appointment', 'woocommerce-appointments' ); ?>
					</button>
				</div>
			</div>
		</form>
		...
		jQuery( document ).on( 'click', '.appointments', function(e) {
		        ...
		        && ap_item.has(e.target).length === 0)
		    {
				...
				if ( $columns_by_staff ) : ?>
					...
					jQuery('.staff_column').each(function(){
				...
				jQuery('.appointments').append('<li id="new_appointment" style="left:' + left + 'px; top:' + top + 'px;"><a><strong class="appointment_datetime">' + hours + ':' + minutes + '</strong><ul><li><?php _e( 'New Appointment', 'woocommerce-appointments' ); ?></li></ul></a></li>');
