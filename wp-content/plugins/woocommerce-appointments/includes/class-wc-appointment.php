...
class WC_Appointment {
	...
	public function __construct( $appointment_data = false ) {
		...
		if ( is_array( $appointment_data ) ) {
			...
			$populated = false;
		} elseif ( is_int( intval( $appointment_data ) ) && 0 < $appointment_data ) {
			$populated = $this->populate_data( $appointment_data );
		} elseif ( is_object( $appointment_data ) && isset( $appointment_data->ID ) ) {
	...
	public function schedule_events() {
		if ( in_array( get_post_status( $this->id ), get_wc_appointment_statuses( 'scheduled' ) ) ) {
			if ( $this->start && $this->get_order() ) {
				...
				if ( ! in_array( $order_status, array( 'cancelled', 'refunded', 'pending', 'on-hold' ) ) && ( $this->start > $now ) ) {
					wp_schedule_single_event( strtotime( '-' . apply_filters( 'woocommerce_appointments_remind_before_time', '1 day' ) . '', $this->start ), 'wc-appointment-reminder', array( $this->id ) );
					wp_schedule_single_event( strtotime( '-' . apply_filters( 'woocommerce_appointments_remind_before_time', '1 day' ) . '', $this->start ), 'wc-appointment-staff-reminder', array( $this->id ) );
				}
			}
			if ( $this->end ) {
			...
			wp_clear_scheduled_hook( 'wc-appointment-staff-reminder', array( $this->id ) );
			wp_clear_scheduled_hook( 'wc-appointment-complete', array( $this->id ) );
		}
	}

	/**
	 * Clear scheduled events for this appointment
	 */
	...
	private function new_appointment( $status, $order_data ) {
		...
		$appointment_data = array(
			...
			'post_title'  => sprintf( __( 'Appointment &ndash; %s', 'woocommerce-appointments' ), strftime( _x( '%1$b %2$d, %Y @ %I:%M %p', 'Appointment date parsed by strftime', 'woocommerce-appointments' ) ) ) . $order_data['summary'],
			'post_status' => $status,
			'ping_status' => 'closed',
		...
		$meta_args = array(
			...
			'_appointment_customer_status'	=> $order_data['customer_status'],
		);

		// Staff IDs.
		$staff_ids = ( $order_data['staff_ids'] ) ? $order_data['staff_ids'] : array( $order_data['staff_id'] );
		foreach ( $staff_ids as $key => $value ) {
			add_post_meta( $this->id, '_appointment_staff_id', $value );
		}
	...
	public function populate_data( $appointment_id ) {
		...
		if ( is_object( $post ) ) {
			...
			foreach ( $load_data as $key => $default ) {
				if ( 'staff_id' == $key && isset( $this->custom_fields[ $meta_prefix . $key ][0] ) && '' !== $this->custom_fields[ $meta_prefix . $key ][0] ) { #because multiple staff can be assigned
					$this->$key = maybe_unserialize( $this->custom_fields[ $meta_prefix . $key ] );
				} elseif ( isset( $this->custom_fields[ $meta_prefix . $key ][0] ) && '' !== $this->custom_fields[ $meta_prefix . $key ][0] ) {
	...
	public function paid() {
		...
		if ( $this->populated && in_array( $current_status, array( 'unpaid', 'confirmed', 'complete' ) ) ) {
	...
	public function update_status( $status ) {
		...
		$allowed_statuses = array_unique( array_merge( get_wc_appointment_statuses( null, true ), get_wc_appointment_statuses( 'user', true ), get_wc_appointment_statuses( 'cancel', true ) ) );
		$allowed_statuses['was-in-cart'] = __( 'Was In Cart', 'woocommerce-appointments' );
		$allowed_status_keys = array_keys( $allowed_statuses );

		if ( $this->populated && in_array( $status, $allowed_status_keys ) ) {
			...
			if ( $order = $this->get_order() ) {
				$order->add_order_note( sprintf( __( 'Appointment #%1$d status changed from "%2$s" to "%3$s"', 'woocommerce-appointments' ), $this->id, $allowed_statuses[ $current_status ], $allowed_statuses[ $status ] ) );
			}
	...
	public function update_customer_status( $status ) {
		$current_status   = get_post_meta( $this->id, '_appointment_customer_status', true );
		$allowed_statuses = get_wc_appointment_statuses( 'customer' );
		$allowed_statuses = array_values( $allowed_statuses );

		if ( $this->populated && in_array( $status, $allowed_statuses ) ) {
			update_post_meta( $this->id, '_appointment_customer_status', $status );

			// Note in the order
			if ( $order = $this->get_order() ) {
				$order->add_order_note( sprintf( __( 'Appointment #%1$d customer status changed from "%2$s" to "%3$s"', 'woocommerce-appointments' ), $this->id, $current_status, $status ) );
			}
	...
	public function get_product_id() {
		$product_id = get_post_meta( $this->id, '_appointment_product_id', true );
	...
	/**
	 * Returns the object of the order corresponding to this appointment
	 * @return WC_Order | Boolean . False if appointment is not populated
	 */
			...
			if ( $order ) {
				...
				if ( is_object( $user ) ) {
					return (object) array(
						...
						'full_name' => esc_html( $user->billing_first_name . ' ' . $user->billing_last_name ),
						'phone' 	=> esc_html( $user->billing_phone ),
						'email'   	=> esc_html( $user->user_email ),
						'user_id' 	=> absint( $user->ID ),
					);
				} else {
					return (object) array(
						'name'      => ( $order->billing_first_name && $order->billing_last_name ) ? esc_html( $order->billing_first_name . ' ' . $order->billing_last_name ) : __( 'Guest', 'woocommerce-appointments' ),
						'full_name' => ( $order->billing_first_name && $order->billing_last_name ) ? esc_html( $order->billing_first_name . ' ' . $order->billing_last_name ) : __( 'Guest', 'woocommerce-appointments' ),
	...
	public function get_staff_id() {
		if ( $this->populated && ! empty( $this->staff_id ) ) {
	...
	public function get_staff_members( $names = false, $with_link = false ) {
		...
		return $product->get_staff_members( $staff_id, $names, $with_link );
	}
}
