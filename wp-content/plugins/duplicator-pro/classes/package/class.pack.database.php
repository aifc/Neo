...
class DUP_PRO_Database
{
	...
	public function doFiltering()
    {
       ...
       // DUP_PRO_LOG::traceObject("tables:", $tables);
      //  DUP_PRO_LOG::trace("tbl filter on: {$tblFilterOn}");
        //Filtering manually selected tables by user
        if (is_array($filterTables) && $this->FilterOn) {
            foreach ($tables as $key => $val) {
                if (!in_array($tables[$key], $filterTables)) {
    ...
    //    DUP_PRO_LOG::traceObject('filtertables', $filterTables);
   //     DUP_PRO_LOG::traceObject('tables to process1', $this->Package->db_build_progress->tablesToProcess);
   ...
   //     DUP_PRO_LOG::traceObject('tables to process2', $this->Package->db_build_progress->tablesToProcess);
    ...
    public function writeInsertChunk()
    {
        ...
        if(count($tables) > 0) {
            ...
            while (!$this->Package->db_build_progress->completed && $elapsed_time<$worker_time){
                DUP_PRO_Log::trace("In while loop");
                $chunk_size = $global->package_phpdump_qrylimit;
                $table = $tables[$current_index];
                $row_count = $wpdb->get_var("SELECT Count(*) FROM `{$table}`");
                $rows_left_to_process = 0;
                if($row_count >= 1){
                    $rows_left_to_process = $row_count-$row_offset+1;
                ...
                DUP_PRO_Log::trace("current_index $current_index");
                DUP_PRO_Log::trace("table $table");
