...
<?php
    ...
    $alert6->message	= DUP_PRO_U::__("No package files are available for direct download from this server. Please use the <b><i class='fa fa-bars small-fa'></i> More Items "
							. "&#10095; Storage option</b> to get the package from it's non-default stored location.<br/><br/>"
							. "<small>Note: To enable the direct download button be sure the local default "
							. "storage type is enabled when creating a package.</small>");
    $alert6->initAlert();
			...
			for (storage_provider_key in data.storage_providers) {
				...
				switch (parseInt(store.storage_type)) {
					...
					case DupPro.Pack.StorageTypes.onedrive:
                        ...
                        info += "<span>Location: <a target=\"_blank\" href=\"" + store.storage_location_string + "\">"+ store.storage_location_string +"</a></span><br/>";
                        info += "</div>";
					break;
				}
			}
