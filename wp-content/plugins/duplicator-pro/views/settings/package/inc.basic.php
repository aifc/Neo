...
<tr>
	...
	<td>
		<input type="radio" name="archive_compression_schedule" id="archive_compression_off_schedule" value="0"	<?php echo DUP_PRO_UI::echoChecked($global->archive_compression_schedule == false); ?> />
		<label for="archive_compression_off_schedule"><?php DUP_PRO_U::_e("Off"); ?></label> &nbsp;

		<input type="radio" name="archive_compression_schedule"  id="archive_compression_on_schedule" value="1"	<?php echo DUP_PRO_UI::echoChecked($global->archive_compression_schedule == true); ?>  />
...
<div id="engine-details-match-message">
	<p class="description">
		<?php DUP_PRO_U::_e("<i style='color:maroon'><i class='fa fa-exclamation-triangle'></i> It is highly recommended that the 'Manual Engine' and 'Scheduled Engine' match. "
			. " Although this option is not required, it is recommended.  For instance, if 'Manual Engine' is set to DupArchive then also set 'Schedule Engine' to DupArchive.</i>"); ?>
	</p>
</div>
...
jQuery(document).ready(function ($)
{
	...
	DupPro.UI.SetBuildType = function(select)
	{
		...
		DupPro.UI.CompareEngineModes();
	}

	DupPro.UI.CompareEngineModes = function()
	{
		var manualEngine =  $("input[type='radio'][name='archive_build_mode']:checked").val();
		var scheduleEngine =  $("input[type='radio'][name='archive_build_mode_schedule']:checked").val();

		if (manualEngine != scheduleEngine) {
			$('#engine-details-match-message').show(200);
		} else {
			$('#engine-details-match-message').hide(200);
		}
	}
	...
	//HOOKS
	$("input[type='radio'][name='archive_build_mode'], input[type='radio'][name='archive_build_mode_schedule']").click(function() {
		DupPro.UI.CompareEngineModes();
	});
