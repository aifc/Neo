...
if (isset($_REQUEST['action'])) {
	...
	if ($_REQUEST['action'] == 'save') {
        ...
        if ($_REQUEST['storage_type'] == DUP_PRO_Storage_Types::OneDrive) {
            if(!empty($storage->onedrive_access_token)){
                echo "inhere";
...
if(isset($_REQUEST['onedrive_action'])){
    if ($_REQUEST['onedrive_action'] == 'save'){
        DUP_PRO_LOG::trace('OneDrive state saving action fired.');
        if ($_REQUEST['storage_type'] == DUP_PRO_Storage_Types::OneDrive) {
            DUP_PRO_LOG::trace('Right Storage Type is set for Onedrive');
            if ($storage->onedrive_authorization_state == DUP_PRO_OneDrive_Authorization_States::Unauthorized) {
                DUP_PRO_LOG::trace("OneDrive hasn't been authorized yet.");
                if (isset($_REQUEST['onedrive_state'])) {
                    DUP_PRO_LOG::trace('onedrive_state variable is set.');
                    $onedrive = json_decode(urldecode($_REQUEST['onedrive_state']));

                    $storage->storage_type                  = DUP_PRO_Storage_Types::OneDrive;
                    $storage->onedrive_access_token         = $onedrive->state->token->data->access_token;
                    $storage->onedrive_refresh_token        = $onedrive->state->token->data->refresh_token;
                    $storage->onedrive_user_id              = $onedrive->state->token->data->user_id;
                    $storage->onedrive_token_obtained       = $onedrive->state->token->obtained;
    ...
    }else if ($_REQUEST['onedrive_action'] == 'onedrive-revoke-access') {
...
if ($storage->onedrive_authorization_state == DUP_PRO_OneDrive_Authorization_States::Authorized) {
    ...
    $onedrive = $storage->get_onedrive_client();
    $onedrive_account_info = $onedrive->fetchAccountInfo();
}
	...
	<tr valign="top">
		...
		<td>
			<select id="change-mode" name="storage_type" onchange="DupPro.Storage.ChangeMode()">
				...
				<?php if($global->profile_beta && DUP_PRO_U::PHP56()) : ?>
			...
			if (DUP_PRO_U::PHP55() == false) {
				echo '<p><i>'.sprintf(DUP_PRO_U::__('SFTP is not available for this version of PHP (%s). Must have PHP 5.5.2 or greater.'), PHP_VERSION).'</i></p>';
			}
			if (!extension_loaded('gmp')) {
				echo '<p><i>'.DUP_PRO_U::__('SFTP is not available for your server, you should enable the gmp extension on your server.').'</i></p>';
			}
			?>
		</td>
	</tr>
    ...
    <tr>
        ...
        <td class="onedrive-authorize">
            ...
            <div class='onedrive-authorization-state' id="onedrivestate-unauthorized">
                <!-- CONNECT -->
                <button id="dpro-onedrive-connect-btn" type="button" class="button button-large" onclick="DupPro.Storage.OneDrive.OnedriveRedirectToAuthUrl();">
            ...
            <div class='onedrive-authorization-state' id="onedrive-state-authorized" style="margin-top:-5px">
                <?php if ($storage->onedrive_authorization_state == DUP_PRO_OneDrive_Authorization_States::Authorized) : ?>
                    <h3>
                        <?php DUP_PRO_U::_e('OneDrive Account'); ?><br/>
...
<?php
	...
	$alert11->title		= 'FTP Error';
	$alert11->message	= DUP_PRO_U::__('Send FTP file test failed. Be sure the full storage path exists.');
	$alert11->initAlert();

    $alert12 = new DUP_PRO_UI_Dialog();
	$alert12->title		= 'S3 Error';
...
<script>
    jQuery(document).ready(function ($) {
        ...
        DupPro.Storage.OneDrive.OnedriveRedirectToAuthUrl = function()
        {
            jQuery('.button_onedrive_test').prop('disabled', true);

            var data = {action: 'duplicator_pro_onedrive_get_auth_url', <?php echo isset($storage->id) ? "storage_id: ".$storage->id : "";  ?> };
            ...
            $.ajax({
                ...
                success: function (data) {
                    // Success
                    if(data['status'] == 0){
                        window.location.replace(data['onedrive_auth_url']);
                        //console.log(data);
                    }
                    else{
        ...
        DupPro.Storage.OneDrive.CancelAuthorization = function(){
            ...
            var data = {action: 'duplicator_pro_onedrive_get_logout_url', <?php echo isset($storage->id) ? "storage_id: ".$storage->id : "";  ?> };
            ...
            $.ajax({
                ...
                success: function (data) {
                    // Success
                    if(data['status'] == 0){
                        window.location.replace(data['onedrive_logout_url']);
                    }
                    else{
                        alert("<?php DUP_PRO_U::_e('Error getting Onedrive logout URL. Please try again later.') ?>");
                    }
                },
                error: function (data) {
                    alert("<?php DUP_PRO_U::_e('Unable to get Onedrive logout URL.') ?>");
                }
            });
        };
