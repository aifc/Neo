...
//                    DUP_PRO_LOG::traceObject("upload infos for $package_id are", $providers);
        ...
        function duplicator_pro_dropbox_send_file_test()
        {
            ...
            try {
                ...
                // ob_start();
                // print_r($source_filepath);
                // echo PHP_EOL;
                // print_r($dropbox_filepath);
                // print_r($upload_result);
                // $data=ob_get_clean();
                // file_put_contents(dirname(__FILE__) . '/upload_result.log',$data,FILE_APPEND);
                /** -- Download the file --* */
                $dest_filepath = tempnam(sys_get_temp_dir(), 'DUP');

                DUP_PRO_LOG::trace("About to download $dropbox_filepath in dropbox to $dest_filepath");
                ...
                // print_r($dropbox_filepath);
                // echo PHP_EOL;
                // print_r($dest_filepath);
                // print_r($dropbox_filemeta);
                // $data=ob_get_clean();
                // file_put_contents(dirname(__FILE__) . '/dropbox_filemeta.log',$data,FILE_APPEND);
...
//        }
        ...
        function get_onedrive_auth_url()
        {
            ...
            $callback_uri = urlencode(self_admin_url("admin.php?page=duplicator-pro-storage&tab=storage".
                "&inner_page=edit&onedrive_action=save&storage_type={$onedrive_storage_type}{$storage_id}"));
            $auth_arr = DUP_PRO_Onedrive_U::get_onedrive_auth_url_and_client($callback_uri);
