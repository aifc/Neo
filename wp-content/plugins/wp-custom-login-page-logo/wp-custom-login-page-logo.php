...
if( array_intersect($current_user_role, $current_user->roles ) && is_admin() && $_POST['wpclpl_save']=='1') { 
...
add_filter('counselor_login_head', 'wpclpl_load_jquery');
...
*
* checks file location (locally or external url)
************************************************
*/
function wpclpl_file_is_local(){
...
function wpclpl_image_dimensions( $return=false ){
	...
	if( !empty( $wpclpl_plugin_options['wpclpl_logo_url'] ) ){
		...
		if( wpclpl_file_is_local() === true ){
			...
			$wpclpl_logo_file_path = $_SERVER['DOCUMENT_ROOT'].'/'.$wpclpl_logo_pathinfo['dirname'].'/'.$wpclpl_logo_pathinfo['basename'];
...
/*
* fcuntion wpclpl_build_css_output()
*
* build css output.
*
...
*
* displays preview of custom css output
**********************************************
*/
/*
function wpclpl_settings_custom_css_preview(){
