...
if ( is_woocommerce_active() ) {
	...
	function get_product_addons( $post_id, $prefix = false, $inc_parent = true, $inc_global = true ) {
		...
		if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '3.0.0', '<' ) ) {
			$product_terms_objects = apply_filters( 'get_product_addons_product_terms', get_the_terms( $post_id, 'product_cat' ), $post_id );
			$product_terms 	= wp_list_pluck( (array) $product_terms_objects, 'term_id' );
		...
		if ( '1' !== $exclude && $inc_global ) {
			...
			$global_addons = $posts_query->query( $args );
