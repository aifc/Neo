...
class WC_Appointments_Integration_Product_Vendors {
	...
	public function render_appointments_dashboard_widget() {
		if ( false === ( $appointments = get_transient( 'wcpv_reports_appointments_wg_' . WC_Product_Vendors_Utils::get_logged_in_vendor() ) ) ) {
			$args = array(
			...
			set_transient( 'wcpv_reports_appointments_wg_' . WC_Product_Vendors_Utils::get_logged_in_vendor(), $appointments, DAY_IN_SECONDS );
		}

		if ( empty( $appointments ) ) {
			echo '<p>' . __( 'There are no appointments available.', 'woocommerce-appointments' ) . '</p>';
		...
		<table class="wcpv-vendor-appointments-widget wp-list-table widefat fixed striped posts">
			<thead>
				<tr>
					...
					<th><?php esc_html_e( 'Scheduled By', 'woocommerce-appointments' ); ?></th>
					<th><?php esc_html_e( 'Order', 'woocommerce-appointments' ); ?></th>
			...
			<tbody id="the-list">
				...
				foreach ( $appointments as $appointment ) {
					...
					?>
					<tr>
						<td><a href="<?php echo get_edit_post_link( $appointment->ID ); ?>" title="<?php esc_attr_e( 'Edit Appointment', 'woocommerce-appointments' ); ?>"><?php printf( __( 'Appointment #%d', 'woocommerce-appointments' ), $appointment->ID ); ?></a></td>

						<td><a href="<?php echo get_edit_post_link( $appointment_item->get_product()->get_id() ); ?>" title="<?php esc_attr_e( 'Edit Product', 'woocommerce-appointments' ); ?>"><?php echo $appointment_item->get_product()->post->post_title; ?></a></td>

						<td>
							<?php
							if ( $appointment_item->get_customer() ) {
							?>
								<a href="mailto:<?php echo esc_attr( $appointment_item->get_customer()->email ); ?>"><?php echo $appointment_item->get_customer()->name; ?></a>
							<?php
							} else {
						...
						<td>
							...
							?>
								<a href="<?php echo admin_url( 'admin.php?page=wcpv-vendor-order&id=' . $appointment_item->order_id ); ?>" title="<?php esc_attr_e( 'Order Detail', 'woocommerce-appointments' ); ?>"><?php printf( __( '#%d', 'woocommerce-appointments' ), $appointment_item->order_id ); ?></a> &mdash; <?php echo WC_Product_Vendors_Utils::format_order_status( $appointment_item->get_order()->get_status() ); ?>
							<?php
							} else {
								esc_html_e( 'N/A', 'woocommerce-appointments' );
							}
						...
						<td><?php echo $appointment_item->get_start_date(); ?></td>
						<td><?php echo $appointment_item->get_end_date(); ?></td>
					</tr>
					<?php
				}
				?>
	...
	public function filter_appointment_products( $item ) {
		...
		if ( is_object( $appointment_item ) && is_object( $appointment_item->get_product() ) && $appointment_item->get_product()->get_id() && in_array( $appointment_item->get_product()->get_id(), $product_ids ) ) {
			return $item;
		}
	}
