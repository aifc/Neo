	...
	<div id="wca-dialog-wrap" class="wp-core-ui" role="dialog" aria-labelledby="wca-dialog-title">
		<form id="wca-dialog">
			<div id="wca-dialog-header">
				<a class="wca-edit-customer"><img class="wca-customer-avatar"  /></a>
				<div id="wca-detail-customer"><?php _e( 'Guest', 'woocommerce-appointments' ); ?></div>
				<button type="button" id="wca-dialog-close"><span class="screen-reader-text"><?php _e( 'Close', 'woocommerce-appointments' ); ?></span></button>
			</div>
			<div id="wca-dialog-content">
				<div id="wca-dialog-inner">
					<?php do_action( 'woocommerce_appointments_before_admin_dialog_content' ); ?>
					<dl id="wca-detail-product">
						<dt><?php _e( 'Product:', 'woocommerce-appointments' ); ?></dt>
						<dd>-</dd>
					</dl>
					<dl id="wca-detail-staff">
						<dt><?php _e( 'Providers:', 'woocommerce-appointments' ); ?></dt>
						<dd>-</dd>
					</dl>
					<dl id="wca-detail-when">
						<dt><?php _e( 'When:', 'woocommerce-appointments' ); ?></dt>
						<dd>-</dd>
					</dl>
					<dl id="wca-detail-duration">
						<dt><?php _e( 'Duration:', 'woocommerce-appointments' ); ?></dt>
						<dd>-</dd>
					</dl>
					<dl id="wca-detail-status">
						<dt><?php _e( 'Status:', 'woocommerce-appointments' ); ?></dt>
						<dd><?php _e( 'Unpaid', 'woocommerce-appointments' ); ?></dd>
					</dl>
					<dl id="wca-detail-cost">
						<dt><?php _e( 'Cost:', 'woocommerce-appointments' ); ?></dt>
						<dd>-</dd>
					</dl>
			...
			<div id="wca-dialog-footer">
				<div id="wca-dialog-cancel">
					<button type="button" class="button">
						<?php _e( 'Close', 'woocommerce-appointments' ); ?>
					</button>
				</div>
				<div id="wca-dialog-update" class="button-group">
					<?php do_action( 'woocommerce_appointments_before_admin_dialog_button' ); ?>
					<button type="submit" id="wca-dialog-submit" name="wca-dialog-submit" class="button button-primary">
						<?php _e( 'Edit Appointment', 'woocommerce-appointments' ); ?>
					</button>
...
<script type="text/javascript">
	jQuery(function() {
		...
		jQuery( document ).on( 'click', '.appointments li', function(e) {
			...
			var detail_when = jQuery( '#wca-detail-when dd' );
			var detail_duration = jQuery( '#wca-detail-duration dd' );
			...
			var appointment_when = appointment.attr( 'data-appointment-when' );
			var appointment_duration = appointment.attr( 'data-appointment-duration' );
			var appointment_qty = appointment.attr( 'data-appointment-qty' );
			var appointment_status = appointment.attr( 'data-appointment-status' );
			var appointment_status_label = appointment.attr( 'data-appointment-status-label' );
			...
			//** DETAILS: start
				...
				// When
				if ( appointment_when ) {
					detail_when.html( appointment_when );
				}

				// Duration
				if ( appointment_duration ) {
					detail_duration.html( appointment_duration );
				}

				// Status
				if ( appointment_status_label ) {
					detail_status.html( appointment_status_label );
				}
				if ( appointment_status ) {
					detail_status.addClass('wca-detail-status wca-detail-status-' + appointment_status);
				}
