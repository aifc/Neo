...
class WC_Email_Appointment_Notification extends WC_Email {
	 ...
	 * @return void
	 */
	public function trigger( $appointment_id, $notification_subject, $notification_message, $attachments = array() ) {
		if ( $appointment_id ) {
			...
			if ( $this->object->get_order() ) {
				...
				$this->recipient = apply_filters( 'woocommerce_email_notification_recipients', $billing_email );
			} else {
				$this->find[]    = '{order_date}';
