...
<fieldset class="wc-appointments-date-picker wc-appointments-date-picker-<?php echo esc_attr( $product_type ); ?> <?php echo implode( ' ', $class ); ?>">
	...
	<div
		class="picker"
		data-duration-unit="<?php echo esc_attr( $duration_unit );?>"
		data-availability="<?php echo esc_attr( wp_json_encode( $availability_rules ) ); ?>"
		data-default-availability="<?php echo $default_availability ? 'true' : 'false'; ?>"
		data-fully-scheduled-days="<?php echo esc_attr( wp_json_encode( $fully_scheduled_days ) ); ?>"
		data-unavailable-days="<?php echo esc_attr( wp_json_encode( $unavailable_days ) ); ?>"
		data-partially-scheduled-days="<?php echo esc_attr( wp_json_encode( $partially_scheduled_days ) ); ?>"
		data-remaining-scheduled-days="<?php echo esc_attr( wp_json_encode( $remaining_scheduled_days ) ); ?>"
		data-padding-days="<?php echo esc_attr( wp_json_encode( $padding_days ) ); ?>"
		data-discounted-days="<?php echo esc_attr( wp_json_encode( $discounted_days ) ); ?>"
		data-restricted-days="<?php echo esc_attr( wp_json_encode( $restricted_days ) ); ?>"
		data-min_date="<?php echo ! empty( $min_date_js ) ? $min_date_js : 0; ?>"
		data-max_date="<?php echo $max_date_js; ?>"
